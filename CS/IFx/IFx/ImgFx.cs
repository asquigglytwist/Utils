...
namespace ASquigglyTwist
{
    ...
    public partial class ImgFx : Form
    {
        ...
        DialogResult UIRes;
        InputBox iBox;
        public static Dictionary<int, string> colorToCSSClassMap = new Dictionary<int, string>();
        const int BMPPixelStepValue = 1;
        ...
        private string ASCIIArt(Bitmap image)
        {
            const int widthLimit = 124;
            if(image.Width > widthLimit)
            {
                image = GetReSizedBitmap(image, widthLimit);
                picPreview.Image = image;
            }
            var sbAsciiArt = new StringBuilder();
            int prevColor = -1;
            var prevChar = 0.GetAsciiChar();
            bool openSpanTag = false;
            for (int h = 0; h < image.Height; h += BMPPixelStepValue)
            {
                sbAsciiArt.Append("<div>");
                for (int w = 0; w < image.Width; w += BMPPixelStepValue)
                {
                    var grayScaleValue = image.GetPixel(w, h).ToGrayScale();
                    if (grayScaleValue < 241)
                    {
                        if (grayScaleValue == prevColor)
                        {
                            sbAsciiArt.Append(prevChar);
                        }
                        else
                        {
                            if (openSpanTag)
                            {
                                sbAsciiArt.Append("</span>");
                            }
                            var cssClass = grayScaleValue.GetCSSClass();
                            var asciiChar = grayScaleValue.GetAsciiChar();
                            sbAsciiArt.AppendFormat("<span class=\"{0}\">{1}", cssClass, asciiChar);
                            prevChar = asciiChar;
                            prevColor = grayScaleValue;
                            openSpanTag = true;
                        }
                    }
                    else
            ...
            var sbHtmlContent = new StringBuilder("<html><head>");
            sbHtmlContent = sbHtmlContent.Append(ExtensionMethods.GetStyleTag()).Append("</head><body>").Append(sbAsciiArt).Append("</body></html>");
        ...
        private void btnConvert_Click(object sender, EventArgs e)
        {
            ...
            timerss = Environment.TickCount - timerss;
    ...
    {
        static char[] _AsciiChars = new char[] { '#', '/', '@', '%', '+', '*', ':', '-', '1', 'l', '|', '$' };

        // [BIB]:  https://stackoverflow.com/a/15028199
        public static int RoundOffTo(this int i, int newBase = 20)
        {
            return ((int)Math.Round((double)i / newBase)) * newBase;
        }

        public static int ToGrayScale(this Color pixelColor)
        {
            int red, green, blue;
            red = ((2 * pixelColor.R) + pixelColor.G + pixelColor.B) / 4;
            green = ((2 * pixelColor.G) + pixelColor.R + pixelColor.B) / 4;
            blue = ((2 * pixelColor.B) + pixelColor.G + pixelColor.R) / 4;
            return ((red + green + blue) / 3).RoundOffTo();
        }

        public static char GetAsciiChar(this int grayScaleValue)
        {
            int index = (grayScaleValue * (_AsciiChars.Length)) / 255;
            index %= _AsciiChars.Length;
            return _AsciiChars[index];
        }

        public static string GetCSSClass(this int grayValue)
        {
            // Dictionary look-up for keys are faster than List.Contains calls.
            if (!ImgFx.colorToCSSClassMap.TryGetValue(grayValue, out var cssClass))
            {
                cssClass = ImgFx.colorToCSSClassMap[grayValue] = "c" + grayValue;
            }
            return cssClass;
        }

        public static StringBuilder GetStyleTag()
        {
            var sbStyleTag = new StringBuilder();
            var knownGrayColors = ImgFx.colorToCSSClassMap;
            sbStyleTag.Append("<style>\nbody{font-family:'Courier New';font-size:5px;zoom:0.5;}");
            foreach (var color in knownGrayColors.Keys)
            {
                sbStyleTag.AppendFormat(".{0}{{color: rgb({1},{1},{1});}}", knownGrayColors[color], color);
            }
            sbStyleTag.Append("\n</style>");
            return sbStyleTag;
        }
